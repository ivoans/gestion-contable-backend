...
import { requireCondicionFiscal } from '../middleware/condicionFiscal';
...
// Solo responsables inscriptos: el Libro IVA no aplica a monotributistas (tienen
// su propia sección). Rutas estáticas /mis-movimientos/... declaradas antes que el
// /:id de abajo para que ningún segmento se confunda con un parámetro. /resumen y
// /tendencia antes que el listado por prolijidad; todos son literales y no colisionan.
const soloRI = requireCondicionFiscal('responsable_inscripto');
router.get('/mis-movimientos/resumen', authenticate, requireRole('cliente'), soloRI, resumenMisMovimientos);
router.get('/mis-movimientos/tendencia', authenticate, requireRole('cliente'), soloRI, tendenciaMisMovimientos);
router.get('/mis-movimientos', authenticate, requireRole('cliente'), soloRI, listarMisMovimientos);
